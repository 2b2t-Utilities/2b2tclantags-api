...
public class ClanTags {
    ...
    public void onChatMessage(ClientChatReceivedEvent event) {
        ClanTagsApi.onClientChatReceivedEvent(event);
    }

    @SubscribeEvent
    public void onChatPacket(PacketReceivedEvent event) {
        if(!(event.getPacket() instanceof CPacketChatMessage)) return;
        ClanTagsApi.onCPacketChatMessage((CPacketChatMessage) event.getPacket());
    }
}
